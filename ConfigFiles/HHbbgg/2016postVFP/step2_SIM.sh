...
#cd /afs/cern.ch/cms/PPD/PdmV/work/McM/submit/HIG-RunIISummer20UL16SIM-03255/
...
# Dump actual test code to a HIG-RunIISummer20UL16SIM-03255_test.sh file that can be run in Singularity
cat <<'EndOfTestFile' > HIG-RunIISummer20UL16SIM-03255_test.sh
#!/bin/bash
...
chmod +x HIG-RunIISummer20UL16SIM-03255_test.sh
...
singularity run -B /afs -B /cvmfs -B /etc/grid-security -B /etc/pki/ca-trust --no-home /cvmfs/unpacked.cern.ch/registry.hub.docker.com/cmssw/$CONTAINER_NAME $(echo $(pwd)/HIG-RunIISummer20UL16SIM-03255_test.sh)
